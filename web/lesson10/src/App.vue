<template>
    <div id="app">
        ...
        <Footer :usersRegistered="Object.keys(users).length" :postsWritten="Object.keys(posts).length"/>
    </div>
</template>
...
export default {
    ...
    beforeCreate() {
        ...
        this.$root.$on("onRegister", (login, name) => {
          if (login.toString().length < 3 || login.toString().length > 16) {
          ...
          if (!/^[a-z]*$/.test(login)) {
            this.$root.$emit("onRegisterValidationError",
                    "Login must contain only lowercase Latin letters");
            return;
          }

          if (name.toString() === "" || name.toString().length > 32) {
            this.$root.$emit("onRegisterValidationError", "Name must be from 1 to 32 symbols");
            return;
          }
